...
- **SQL Schema Tab:** Shows the `CREATE TABLE` statements read straight out of the loaded database, so students can compare the diagram, the DSE notation and the SQL that produced both. *Simplified* rebuilds them in one consistent shape carrying only what the syllabus asks for - types, `NOT NULL`, `UNIQUE`, `DEFAULT`, `PRIMARY KEY`, `FOREIGN KEY`, and any named `CONSTRAINT ... CHECK` - while the toggle shows the file exactly as written. Works for a file a student loads themselves; for the sample databases it also links to the complete SQL source, rows included.
- **Shareable Links:** `?db=dbviz-05-book-loan.db` opens one particular exercise, so a class can be pointed at the same database; the address bar follows whatever is chosen from the drop-down.
